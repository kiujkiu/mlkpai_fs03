...
#define PVS_FLAG_LZ4       (1u << 5)  /* 载荷 = LZ4 **raw block**。v3.3 起的首选
                                       ...
                                       *
                                       * ---- 2026-08-24: 板端可能由 **PL 硬件**解 ----
                                       * pov_rxd --pl-lz4 会把 lz4 流交给 PL 里的
                                       * lz4_axi_top (dr1v90/lz4hw), 由它**直写帧 bank**
                                       * —— dec 和 staging->bank 那次 memcpy 一起消失。
                                       * 对**发送端**的影响只有两条, 协议本身一个字节
                                       * 都没变:
                                       *   1) PL 只认 lz4。zlib/RLE/raw 帧板端会响亮地
                                       *      回退 CPU (能跑, 但没有加速)。
                                       *   1b) 板端默认把「收下一帧」与「本帧
                                       *      PL 解码」**流水线**起来 (板子只有
                                       *      WiFi 且插不了网线 —— 屏在转 ——
                                       *      收一帧 55-80 ms, 不重叠就把 PL 省
                                       *      下的一半吃回去)。⇒ **ACK 的语义
                                       *      变成「已交给硬件」而不是「已显示」**,
                                       *      解码错误的报告会**晚一帧**: 线上那
                                       *      个 NAK 字节对应刚发出的帧, 真正坏
                                       *      的是上一帧。发送端行为不用改 (收到
                                       *      NAK 照旧重连+keyframe), 但排查现场
                                       *      时要知道这件事; 板端日志会把两个
                                       *      seq 都打出来。
                                       *   2) **DELTA 与 PL 互斥**: PL 的输出落在写合并
                                       *      内存里, XOR 要把上一帧读回来, WC 读极慢
                                       *      ⇒ 板端见到 DELTA 帧就整体退回 CPU 路径
                                       *      (会先 NAK 一次, 发送端重连+keyframe 后恢复)。
                                       *      想吃 PL 的速度就别开 --delta。 */
#define PVS_FLAG_MSTREAM   (1u << 6)  /* 载荷 = **可变条数**的独立压缩流 + 一张流表。
                                       ...
                                       *     两流分支, 逐字节兼容。
                                       *
                                       * ---- 2026-08-24: 流数 = **PL 解码并行度** ----
                                       * 板端 --pl-lz4 打开后, 每条流交给一个 PL 引擎,
                                       * 一条流一个引擎, 谁先空谁接下一条。
                                       * 🔴 **PL 拆不开单条 lz4 流** (LZ4 是串行依赖的
                                       * 回溯格式), 所以并行度**完全**由这里的 n_streams
                                       * 决定 —— 单流帧再多引擎也只有一个在干活。
                                       * ⇒ 板上有 E 个引擎时, 发送端应当切成**正好 E 条
                                       *   等分流** (BD 定稿 E=3 ⇒ 95/95/94 片)。硬件引擎
                                       *   同构, 不像双核那样还要考虑 dec_plan 的连续分组。
                                       *   🔴 老的 `--stream-split balanced` 是当年给**两个
                                       *   CPU 核**调的 (fold540 下 180/90/270 不等分), 对
                                       *   同构硬件引擎是错的 —— 需要 N 等分模式。
                                       * 🔴 而且**每条流自己的 raw_len 必须对**: 板端引擎在
                                       *   "源字节耗尽而 raw_len 还没到"时会 busy 恒 1 卡死
                                       *   (不置 error 也不置 done), 而 RTL 没有软复位 ⇒ 一个
                                       *   引擎就此永久报废, 只能重新加载比特流。上面那两个
                                       *   求和自校验**管不到单条流**, 别把它们当保险。
                                       * 这也正是当初为双核并行切多流的那个设计**白捡**
                                       * 的好处: 换成多个硬件引擎, 编码侧一行都不用动。 */
